...
function ddx_up(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
...
function ddy_up(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
        return CPUVectorOps.ddy_up(field, result, dx, order)
    elseif backend isa GPU
        return GPUVectorOps.ddy_up(field, result, dx, order)
    else
        throw(ArgumentError("Unsupported backend: $backend"))
    end
end

function ddz_up(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
        return CPUVectorOps.ddz_up(field, result, dx, order)
    elseif backend isa GPU
        return GPUVectorOps.ddz_up(field, result, dx, order)
    else
        throw(ArgumentError("Unsupported backend: $backend"))
    end
end

function ddx_dn(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
...
function ddy_dn(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
        return CPUVectorOps.ddy_dn(field, result, dx, order)
    elseif backend isa GPU
        return GPUVectorOps.ddy_dn(field, result, dx, order)
    else
        throw(ArgumentError("Unsupported backend: $backend"))
    end
end

function ddz_dn(backend::Backend, field, result, dx, order::Int)
    if backend isa CPU
        return CPUVectorOps.ddz_dn(field, result, dx, order)
    elseif backend isa GPU
        return GPUVectorOps.ddz_dn(field, result, dx, order)
    else
        throw(ArgumentError("Unsupported backend: $backend"))
    end
end
